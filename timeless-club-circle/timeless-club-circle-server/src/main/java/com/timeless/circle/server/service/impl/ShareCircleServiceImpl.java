...
import com.timeless.circle.server.dao.ShareCircleMapper;
import com.timeless.circle.server.entity.po.ShareCircle;
import com.timeless.circle.server.service.ShareCircleService;
...
public class ShareCircleServiceImpl extends ServiceImpl<ShareCircleMapper, ShareCircle> implements ShareCircleService {
    ...
    public List<ShareCircleVO> listResult() {
        ...
        return Optional.ofNullable(res).orElseGet(() -> {
            List<ShareCircle> list = super.list(Wrappers.<ShareCircle>lambdaQuery().eq(ShareCircle::getIsDeleted,
                    IsDeletedFlagEnum.UN_DELETED.getCode()));
            List<ShareCircle> parentList =
                    list.stream().filter(item -> item.getParentId() == -1L).collect(Collectors.toList());
    ...
    public Boolean updateCircle(UpdateShareCircleReq req) {
        LambdaUpdateWrapper<ShareCircle> update = Wrappers.<ShareCircle>lambdaUpdate().eq(ShareCircle::getId,
                        req.getId())
