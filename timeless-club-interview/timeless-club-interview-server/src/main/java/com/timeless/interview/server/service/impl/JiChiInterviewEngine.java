...
import com.timeless.interview.server.entity.po.SubjectCategory;
import com.timeless.interview.server.entity.po.SubjectInfo;
import com.timeless.interview.server.entity.po.SubjectLabel;
import com.timeless.interview.server.service.InterviewEngine;
import com.timeless.interview.server.util.EvaluateUtils;
...
public class timelessInterviewEngine implements InterviewEngine {
    ...
    public void init() {
        ...
        categoryMap = subjectDao.listAllCategory().stream().collect(Collectors.toMap(SubjectCategory::getId,
                Function.identity()));
    }
    ...
    public InterviewVO analyse(List<String> KeyWords) {
        ...
        List<InterviewVO.Interview> views =
                this.labels.stream().filter(item -> KeyWords.contains(item.getLabelName())).map(item -> {
                    ...
                    if (Objects.nonNull(subjectCategory)) {
                        interview.setKeyWord(String.format("%s-%s", subjectCategory.getCategoryName(),
                                item.getLabelName()));
                    } else {
    ...
    public InterviewQuestionVO start(StartReq req) {
        ...
        List<Long> ids =
                req.getQuestionList().stream().map(StartReq.Key::getLabelId).distinct().collect(Collectors.toList());
