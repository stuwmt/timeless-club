...
import com.timeless.interview.server.dao.InterviewHistoryDao;
...
import com.timeless.interview.server.service.InterviewHistoryService;
...
public class InterviewHistoryServiceImpl extends ServiceImpl<InterviewHistoryDao, InterviewHistory> implements InterviewHistoryService {
    ...
    public void logInterview(InterviewSubmitReq req, InterviewResultVO submit) {
        ...
        String keyWords =
                req.getQuestionList().stream().map(InterviewSubmitReq.Submit::getLabelName).distinct().collect(Collectors.joining("、"));
